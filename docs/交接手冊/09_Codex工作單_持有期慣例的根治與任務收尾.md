...
#### ⚠️ 2026-09-18 追加：重跑 threshold_sweep 後要驗一件事

`threshold_sweep_dev.json` 現在顯示 E1 與 E2 六個方案的報酬**完全相同**
（動能突破全部 +56.50%，籌碼跟隨全部 +7.10%）。原因是 3 槽位 × 60 日
持有把成交數鎖在 63 筆——門檻站在瓶頸下游，改訊號數不改結果。

那是待辦第 2 項的結論依據（見
`../../qlib-tw-trader/docs/原理說明/2026-09-18_門檻形同虛設與路徑相依的根因.md`）。

```
□ 重跑後確認 E1/E2 的報酬是否**仍然彼此相同**
```

若 join 修正讓它們開始分歧，代表槽位上限不再是唯一瓶頸，
**那個結論就要重看，請直接回報不要自己下判斷。**

⚠️ 重跑時 E3 那一列是主線唯一還在用的路徑（比值 1.0）。
E1/E2 已標記為僅供重現歷史，不要為了讓它們「看起來有效」而調參數。
...
#### ⚠️ 2026-09-18 追加：Sharpe 要附 bootstrap 區間，點估計不算答案

當天實測：樣本外累積 +138.83% 的 95% 區間是 `[−13.34%, +587.23%]`，
**報告自帶的七個基準全部落在裡面**。累積報酬與 Sharpe 在 n≤36 下
不可 falsify。

所以「兩個 sleeve 的 Sharpe 有沒有改善」問成點估計沒有答案。要問的是：

```
□ 單 sleeve 與雙 sleeve 的 Sharpe 各自的 bootstrap 95% 區間
□ 兩者的**配對差異**區間（逐期相減，這個才會夠窄）
□ 區塊長度掃 (1, 2, 4)，並報告對長度的敏感度
```

配對差異存活、累積量不存活的原因：逐期相減消掉市場共同變動，
配對 SE 是 1.11% 而單組每趟 SE 是 1.97%；而複利把每期變異放大，
16~36 次乘法後尾部極長。

工具已經有了，直接用：

```
taiwan_quant/validation/bootstrap.py       block_bootstrap、paired_difference
scripts/diagnose_bootstrap_intervals.py    兩種報告格式都吃
```

⚠️ `block_length` 與 `seed` 都是必填，沒有預設值。**種子要寫進報告**，
否則區間無法重現。

⚠️ **sleeve 報告必須存逐期序列**（`per_period` 或 `series`）。
CLAUDE.md 2026-09-18 新增的規格——沒存的話那份結果的不確定性只能
靠重跑取得，而重跑是禁令 6 擋住的。`+1076%`（籌碼跟隨 × 60 日）
已經是這個狀態，永久算不出區間。
...
□ 任務 Y：Sharpe 要報淨值 + bootstrap 區間 + 配對差異區間，點估計不算答案
□ 任務 Y：sleeve 報告必須存逐期序列，否則區間永久算不出來
