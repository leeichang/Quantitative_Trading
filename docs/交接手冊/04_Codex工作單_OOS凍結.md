...
> **狀態（2026-09-14）**：任務 A、B、C 已完成，任務 D 未做。
> 交付後修了四個缺陷：訓練窗口凍結（Codex 自己修，`--freeze-model`）、
> 帳本沒存 `edge_z`、`BDay` 算錯揭曉日、重複索引。
> 見 [05_Codex工作單_門檻與評估.md](05_Codex工作單_門檻與評估.md) 第一部分與
> `qlib-tw-trader/docs/原理說明/2026-09-14_OOS凍結與前推帳本原理.md`。
> **接下來請看 05，不是這一張。**
