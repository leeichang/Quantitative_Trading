...
**輸 0050 買進持有（+242.15%、Sharpe 1.90）**。

⚠️ 2026-09-18 撤回：原本這裡還寫「超過隨機 10 檔的 95% 分位
（+120.97%）」。那個門檻來自弱虛無（隨機抽 10 檔），而規格已改成
嚴格虛無（稀疏隨機權重）。換算到 OOS 時加法與乘法兩種搬法給出相反
判定，真測需在 OOS 跑 200 組——禁令 6 禁止。該句降級為「量不出差別」。
見 ../../qlib-tw-trader/docs/原理說明/2026-09-18_超過隨機95分位這句話該退休.md

⚠️ **帳本目前記的對照組是弱虛無。** 第一期 2026-11-09 揭曉前要補記
嚴格虛無的同期抽樣——只用決策日當天為止的特徵算權重，不看未來報酬，
所以不算用掉區間。
