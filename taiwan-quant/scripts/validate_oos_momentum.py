...
```
毛報酬          4.89%/趟（隨機 10 檔 2.33%，隨機 95% 分位 3.11%）← 已降級，見下
逐年為正        7/8 年，最差 −1.6%
CPCV            15 條路徑全為正，最差 +3%
...
⚠️ 2026-09-18 降級：上表第一項的 `隨機 95% 分位 3.11%` 來自弱虛無
（隨機抽 10 檔）。規格已改成嚴格虛無（同特徵集的稀疏隨機權重），
開發集實測毛 3.48%／淨 2.393%。邊際從 +1.06 pp 縮到 +0.69 pp，
而配對 SE 是 2.364 pp——**0.29 SE，量不出差別**。

且 `4.89%/趟` 另含持有期多算一天，修正後為 4.17%/趟。兩個修正方向相同。

**所以開發集是七項通過五項，不是六項。** DSR 仍是唯一直接針對過度
擬合的檢定，而它不通過。
