...
def run(db_path: Path, end: date) -> dict:
    ...
    for day in decision_dates:
        ...
        def record(bucket: list, picks: list[str]) -> None:
            gross = float(realized[picks].mean())
            cost = period_cost(picks, entry_day, raw_opens, opens, large_members)
            bucket.append({"decision_date": str(day.date()), "gross": gross,
