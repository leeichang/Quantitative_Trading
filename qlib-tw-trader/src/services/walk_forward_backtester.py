...
from config.costs import DEFAULT as DEFAULT_COST
from config.costs import CostModel, Tier
...
@dataclass
class WeekReturnBreakdown:
    """
    單週收益拆解（毛/淨並列）

    原本 _calculate_week_return 只回傳毛報酬，完全不扣交易成本，
    導致 API 與 Dashboard 顯示的數字與 scripts/evaluate_models.py
    （有扣成本）不一致。本結構讓兩者同時可見。
    """
    ...
    """淨報酬（%），扣除 config/costs.py 的手續費、證交稅、滑價"""

    market_return: float | None
    """市場等權報酬（%）"""

    one_way_turnover: float
    """本週單邊換手率累計（1.0 = 換掉一次完整部位）"""
...
class WeekResult:
    ...
    gross_return: float | None = None  # 毛報酬（%），供毛淨對照
    one_way_turnover: float | None = None  # 單邊換手率
    cost_pct: float | None = None  # 交易成本（%）
    rebalance_count: int | None = None  # 調倉次數
...
class WalkForwardBacktester:
    ...
    def run(
        ...
        for i, info in enumerate(model_infos):
            ...
            try:
                ...
                # 計算週收益（日度調倉，close-to-close），毛淨並列
                breakdown = self._calculate_week_return(
                    predictions, predict_start, predict_end, max_positions,
                    cost=cost, tier=tier,
                )
                # week_return 對外一律代表「淨報酬」，毛報酬另存欄位供對照
                week_return = breakdown.net_return
                market_return = breakdown.market_return
                ...
                weekly_results.append(WeekResult(
                    ...
                    one_way_turnover=breakdown.one_way_turnover,
                    cost_pct=breakdown.cost_pct,
                    rebalance_count=breakdown.rebalance_count,
                ))
    ...
    def _calculate_week_return(
        ...
        成本處理：每個調倉日比對 Top-K 組成變動，算出單邊換手率，
        再依 config/costs.py 的費率扣款。換手率 = 被換掉的檔數 / K。
        ...
        Args:
            ...
            predict_end: 預測期結束日期
            max_positions: 最大持倉數
            cost: 成本模型；None 表示用 config/costs.py 的 DEFAULT
            tier: 流動性分層，決定滑價

        Returns:
            WeekReturnBreakdown（毛報酬、淨報酬、市場、換手率、成本）
        """
        cost = cost if cost is not None else DEFAULT_COST
        empty = WeekReturnBreakdown(None, None, None, 0.0, 0.0, 0)
        ...
        portfolio_daily = []
        market_daily = []
        daily_cost_rate: list[float] = []  # 每個調倉日的成本率
        turnover_total = 0.0  # 單邊換手率累計
        rebalance_count = 0
        ...
        for pred_date in sorted(predictions.index):
            ...
            # 換手率與成本：比對本次 Top-K 與上一次的組成差異
            holdings = set(topk_in_common)
            if prev_holdings is None:
                # 首次建倉：全部買進，只有買方成本
                turnover = 1.0
                cost_rate = cost.one_way_rate(is_sell=False, tier=tier)
            else:
                replaced = len(holdings - prev_holdings)
                turnover = replaced / max(len(holdings), 1)
                # 換掉一檔 = 賣舊 + 買新，兩邊都要收
                cost_rate = turnover * cost.round_trip_rate(tier)

            turnover_total += turnover
            daily_cost_rate.append(cost_rate)
            rebalance_count += 1
        ...
        for r in portfolio_daily:
            week_compound *= (1 + r)
        gross_return = (week_compound - 1) * 100

        # 扣成本後的淨報酬：每個調倉日的報酬先扣掉當日成本率再複合
        net_compound = 1.0
        for r, c in zip(portfolio_daily, daily_cost_rate):
            net_compound *= (1 + r - c)
        net_return = (net_compound - 1) * 100
        ...
        return WeekReturnBreakdown(
            gross_return=float(gross_return),
            net_return=float(net_return),
            market_return=float(market_return),
            one_way_turnover=float(turnover_total),
            cost_pct=float(sum(daily_cost_rate) * 100),
